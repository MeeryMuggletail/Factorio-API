...
function gui_hud.render_signals(hud_combinator, parent_gui, max_columns, signals_filter)
		...
		if networks[i] and networks[i].signals then
			...
			for j, signal in pairs(networks[i].signals) do
				...
				if common.short_if(should_filter, filter_signal(signals_filter, signal_name), true) then
					local signalKey = const.SIGNAL_TYPE_MAP[signal_type]
					local typeColor = network_colors[i]
					local tempTable = {
						type = "sprite-button",
						sprite = signalKey .. "/" .. signal_name,
						number = signal.count,
						style = network_styles[i],
						tooltip = signal_name_map[signal_type][signal_name].localised_name
					}
					table["table"].add(tempTable)

					jsonTable[typeColor] = jsonTable[typeColor] or {}
					jsonTable[typeColor][signalKey] = jsonTable[typeColor][signalKey] or {}
					jsonTable[typeColor][signalKey][signal_name] = jsonTable[typeColor][signalKey][signal_name] or {}

					jsonTable[typeColor][signalKey][signal_name] = signal.count
	...
	json = game.table_to_json(jsonTable)
	game.write_file(hud_combinator.name .."_mqtt.json", json, false)
	return {
